...
public class CloudDepartmentDaoImpl extends SimpleHibernateRepository<CloudDepartment, String> implements ICloudDepartmentDao {
	...
	private Map<String, Object> getDeptOrgList(String tenantId, String deptId, List<Integer> beenDeletedFlags) {
		...
		if (StringUtils.isBlank(deptId)) {
			sql.append(" SELECT dept.tenantId, -1 parentId, dept.id departmentId, dept.depType type,'" + CompanyTypeEnum.DEPART.getKey() + "' companyType   ,");
			sql.append(" 	dept.id, dept.depName name, dept.depCode code, dept.lngLats   , dept.beenDeleted,dept.deletedTime,dept.createTime,dept.lastChangeTime                 ");
			sql.append("     ,dept.email,                                ");
			sql.append("     	dept.address,                           ");
			sql.append("     	dept.lngLats,                           ");
			sql.append("     	dept.description,                       ");
			sql.append("     	dept.headMobile,                        ");
			sql.append("     	dept.head,dept.orderIndex,dept.divisionId                               ");

			sql.append(" FROM  (  SELECT  dept.*  ");
			sql.append(" FROM cloud_department     dept                                    ");
			...
			sql.append(" ORDER BY dept.orderIndex ASC ) dept  ");
			sql.append("                                                               ");
			sql.append(" 	UNION                                                        ");
		}
		...
		sql.append("     	org.head,org.orderIndex,org.divisionId                               ");
		sql.append(" FROM (                                                        ");
		...
		sql.append("  ORDER BY org.orderIndex ASC  ");
	...
	private Map<String, Object> getSyncDeptOrgList(String tenantId, long syncTime, Integer pageSize, Integer pageNumber) {
		...
		if (syncTime != ZERO_LONG) {
			// sql.append(" AND ((UNIX_TIMESTAMP(dept.createTime)*1000> ? and
			// dept.beenDeleted = ? ) ");
			// sql.append(" or (UNIX_TIMESTAMP(dept.lastChangeTime)*1000> ? and
			// dept.beenDeleted = ? and UNIX_TIMESTAMP(dept.createTime)<= ? ) ");
			// sql.append(" or (UNIX_TIMESTAMP(dept.deletedTime)*1000> ? and
			// dept.beenDeleted = ? and UNIX_TIMESTAMP(dept.createTime)<= ? )) ");
			// argList.add(syncTime);
			// argList.add(BakDeleteModel.NO_DELETED);
			// argList.add(syncTime);
			...
			// argList.add(BakDeleteModel.DELETED);
			// argList.add(syncTime);
			sql.append(" AND UNIX_TIMESTAMP(dept.lastChangeTime)*1000> ?  ");
			argList.add(syncTime);
		}
		sql.append(" ) as  mm ");
		...
		if (syncTime != ZERO_LONG) {
			...
			// = ? ) ");
			// sql.append(" or (UNIX_TIMESTAMP(org.lastChangeTime)*1000> ? and
			// org.beenDeleted = ? and UNIX_TIMESTAMP(org.createTime)<= ? ) ");
			// sql.append(" or (UNIX_TIMESTAMP(org.deletedTime)*1000> ? and org.beenDeleted
			// = ? and UNIX_TIMESTAMP(org.createTime)<= ? )) ");
			// argList.add(syncTime);
			// argList.add(BakDeleteModel.NO_DELETED);
			// argList.add(syncTime);
			// argList.add(BakDeleteModel.NO_DELETED);
			...
			sql.append(" AND UNIX_TIMESTAMP(org.lastChangeTime)*1000> ? ");
			argList.add(syncTime);
		}
	...
	public List<TenantDeptOrgDto> findDeptList(String tenantId, String deptId) {
		...
		if (StringUtils.isBlank(deptId)) {
			...
			sql.append(" WHERE tenantId = ?                                            ");
			sql.append(" 	AND beenDeleted = ?                                          ");
			sql.append(" ORDER BY orderIndex asc   ");
			argList.add(tenantId);
			argList.add(BakDeleteModel.NO_DELETED);
		}
		return jdbcTemplate.query(sql.toString(), argList.toArray(), BeanPropertyRowMapper.newInstance(TenantDeptOrgDto.class));
	...
	@Override
	public CloudDepartment getByCode(String tenantId, String deptCode, Integer deletedFlag) throws Exception {
		List<SearchFilter> filterList = new ArrayList<SearchFilter>();
		filterList.add(new SearchFilter("depCode", Operator.EQ, deptCode));
		...
		if (deletedFlag == null) {
			...
			filterList.add(new SearchFilter("beenDeleted", Operator.EQ, deletedFlag));
		}
