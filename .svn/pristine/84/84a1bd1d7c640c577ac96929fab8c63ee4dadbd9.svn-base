...
public class CopyFunctionAcrossDatabaseServiceImpl implements ICopyFunctionAcrossDatabaseService {
	...
	public void coyp(String sourceSystemId, String targetSystemId, List<String> smenus) throws Exception {
		if (StringUtils.isEmpty(sourceSystemId) || StringUtils.isEmpty(targetSystemId)
				|| CollectionUtils.isEmpty(smenus)) {
			return;
		}
		...
		// 发送消息，更新目标management缓存
		String url_target = PropertyUtils.getPropertyValue(ManagementConstant.COPY_RESOURCE_TARGET_URL);
		if (!StringUtil.isNullOrEmpty(url_target)) {
			url_target = getBaseUrl(url_target,
					"/cloud/management/rest/np/menu/refresh" + ManagementConstant.PERMISSION_SUFFIX_READ);
			Map<String, Object> prams = Maps.newHashMap();
			prams.put("systemId", targetSystemId);
			callRestService.callRest(url_target, prams, ManagementConstant.METHOD_GET);
		}
	}

	private static final String URL_SPLIT = "/";

	private String getBaseUrl(String ip, String url) {
		if (URL_SPLIT.equals(ip.substring(ip.length() - 1, ip.length()))) {
			ip = ip.substring(0, ip.length() - 1);
		}
		if (URL_SPLIT.equals(url.substring(0, 1))) {
			url = url.substring(1, url.length());
		}
		return ip + URL_SPLIT + url;
	}
	...
	private void addMenu(String targetSystemId, CloudMenu om) throws Exception {
		...
		for (int i = oldMenus.size() - 1; i >= 0; i--) {
			...
			if (newMenu == null) {
				...
				newMenu.setIconFont(oldMenus.get(i).getIconFont());//图标
				...
				if (!ROOT_ID.equals(parentId)) {
					...
					newParentMenu.setChildSerialNumer(
							newParentMenu.getChildSerialNumer() == null ? 1 : newParentMenu.getChildSerialNumer() + 1);
	 ...
	 * @throws Exception
	 */
	private void getWaitCopyMenus(String targetSystemId, CloudMenu oldMenu, List<CloudMenu> rst, List<String> parentIds)
			throws Exception {
		rst.add(oldMenu);
	 ...
	 * @throws Exception
	 */
	private void getOldGroups(String targetSystemId, CloudFunctionGroup oldGroup, List<CloudFunctionGroup> rst,
			List<String> pids) throws Exception {
		rst.add(oldGroup);
		// 找到新库中同code的组
		CloudFunctionGroup newGroup = cloudFunctionGroupDao2.getFunctionGroupBySysidAndFgcode(targetSystemId,
				oldGroup.getCode());
	...
	private void updateMenu(String targetSystemId, CloudMenu om, CloudMenu nm) throws Exception {
		...
		if (!newMenuIds.contains(nm.getId())) {
			...
			nm.setIconFont(om.getIconFont());//图标
	...
	private String addFunction(String targetSystemId, CloudFunction of) throws Exception {
		...
		if (CloudFunction.FUNCTION_TYPE_MINOR.equals(of.getFunctionType())
				&& StringUtils.isNotEmpty(of.getMainFunctionId())) {
			CloudFunction oldMainFunction = cloudFunctionDao.getByCode(of.getSystemId(), of.getMainFunctionId());
			CloudFunction newMainFunction = cloudFunctionDao2.getFunctionBySysidAndFcode(targetSystemId,
					oldMainFunction.getCode());
	 ...
	 * @throws Exception
	 */
	private String copyGroup(String targetSystemId, List<CloudFunctionGroup> waitCopiedOldGroups, String firstParentId)
			throws Exception {
		String rst = null;
		...
		for (int i = waitCopiedOldGroups.size() - 1; i >= 0; i--) {
			// 找到新库中同code的组
			newGroup = cloudFunctionGroupDao2.getFunctionGroupBySysidAndFgcode(targetSystemId,
					waitCopiedOldGroups.get(i).getCode());
