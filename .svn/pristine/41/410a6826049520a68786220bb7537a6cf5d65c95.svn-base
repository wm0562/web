...
public class UploadResultInfoController extends BaseController {
	...
	public RestResultDto<DataStore<UploadResultInfo>> pageList(HttpServletRequest request) {
		try {
			...
			String marks = centralCacheRedisService.getObject(ManagementConstant.MARK_KEY_PREFIX + (StringUtils.isEmpty(SpringmvcUtils.getParameter("userId")) ? super.getLoginInfo(request).getUserId() : SpringmvcUtils.getParameter("userId")),
					String.class);
