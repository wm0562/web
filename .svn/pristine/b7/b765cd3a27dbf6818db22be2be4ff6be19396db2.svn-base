...
public class OrganizationTreeWithPermission extends CommonTree {
	 ...
	 * @param filterList
	 */
	public void reloadOrganizationElementTree(WorkElementLoginInfoDto info, List<SearchFilter> filterList) {
		...
		try {
			...
			// 切地图处理
			this.changeMapDeal(workElements, info.getMapType());
	 ...
	 * @param mapType
	 */
	private void changeMapDeal(List<WorkElementDto> workElements, String mapType) {
		Tenant tenant = null;
		if (CollectionUtils.isNotEmpty(workElements)) {
			tenant = this.getTenantService().findOne(workElements.get(0).getTenantId());
			IMapUtilService util = this.getMapUtilService();
			List<String> mapTypes = util.getMapTypes(tenant.getMapDefJson());
			...
			String tenantMapType = util.getMapType(tenant.getMapDefJson()); // 租户默认地图类型
			String tenantMapcoord = util.getCoordType(tenant.getMapDefJson()); // 租户默认坐标系
			...
			if (MapTypeEnum.ARCGIS.getKey().equals(currMapType)) {
				currCoord = util.getArcgisCoord(tenant.getMapDefJson());
			...
			for (WorkElementDto dto : workElements) {
				String mapJson = dto.getMapJson();
				...
				if (llr == null) {
					if (!tenantMapType.equals(currMapType)) {
						dto.setParamsDone(util.getParams(dto.getParamsDone(), tenantMapcoord, currCoord));
					}
				} else {
					if (Map2CoordEnum.AMAP.getValue().equals(currCoord)) {
						dto.setParamsDone(llr.getGcj02());
					} else if (Map2CoordEnum.BMAP.getValue().equals(currCoord)) {
						dto.setParamsDone(llr.getBd09());
					} else {
						dto.setParamsDone(llr.getWgs84());
					}
				}
			}
		}
	...
	public void reloadOrganizationElementTree(String tenantId, String userId, List<SearchFilter> filterList) {
		try {
			String isControlPermission = "1";
			// // 获取用户
			// CloudUser user = getCloudUserService().findOne(userId);
			...
			// // 人员信息
			// CloudStaff staffDto =
			// getCloudStaffService().findOne(user.getStaffId());
			// String orgId = null;
			// String departmentId = null;
			// String companyId = null;
			// if (staffDto != null) {
			...
			// 不控制权限或者拥有的是全部权限，显示该租户下的
			if (StringUtils.isEmpty(permissionScope) || PermissionScopeEnum.ALL.getKey().equals(permissionScope) || StringUtils.isBlank(isControlPermission) || "0".equals(isControlPermission)) {
				...
				List<TenantDeptOrgDto> selected = this.getDeptOrgByIds(tenantId, customScope);

				// 租户下所有的depart和org
				Map<String, TenantDeptOrgDto> allMap = Maps.newHashMap();
				List<TenantDeptOrgDto> all = this.findDeptOrgList(tenantId, null);
				// 利用map,去除重复
				if (CollectionUtils.isNotEmpty(all)) {
					for (TenantDeptOrgDto tenantDeptOrgDto : all) {
						allMap.put(tenantDeptOrgDto.getId(), tenantDeptOrgDto);
				...
				if (StringUtils.isNotBlank(companyId)) {
					...
					// 返回自己属于的节点，挂在root节点下
					TenantDeptOrgDto tenantDeptOrgDto = this.getCompany(tenantId, companyId);
					tenantDeptOrgDto.setParentId("-1");
					if (tenantDeptOrgDto != null) {
						nodes.add(tenantDeptOrgDto);
						permissionCompanyNodes.add(tenantDeptOrgDto);
				...
				try {
					...
					if (StringUtils.isNotBlank(companyId)) {
						// 本身
						TenantDeptOrgDto self = this.getCompany(tenantId, companyId);
						// 将本身挂在root下，所以父节点设为-1
						self.setParentId(DepartmentTree.ROOT_NODE_ID);
	...
	private ITenantService getTenantService() {
		return SpringContextHolder.getBean("tenantService");
	}

	private IMapUtilService getMapUtilService() {
		return SpringContextHolder.getBean("mapUtilService");
	}
}
