...
public class CloudFunctionDaoImpl extends SimpleHibernateRepository<CloudFunction, String> implements ICloudFunctionDao {
	...
	public CloudFunctionDto getFunctionById(String id) {
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT cf.*, cfg.`name` groupName, s.systemName goalSystemName, cf2. NAME mainFunctionName ");
		sql.append(" FROM cloud_function cf ");
		sql.append(" JOIN cloud_function_group cfg ON cfg.id = cf.groupId ");
		sql.append(" LEFT JOIN cloud_function cf2 ON cf2.id = cf.mainFunctionId ");
		sql.append(" LEFT JOIN cloud_system s ON s.id = cf.goalSystemId ");
		sql.append(" WHERE cf.id = ? ");
	...
	public List<CloudFunctionAndroidDto> getFunctionsByUserId(String userId) {
		...
		sql.append(" from cloud_user_role a ");
		sql.append(" join cloud_function_role b on a.roleId=b.roleId");
		sql.append(" join cloud_function c on b.functionId=c.id");
		sql.append(" where a.userId=? ");
		sql.append("   and a.beenDeleted=? ");
		sql.append("   and b.beenDeleted=? ");
		sql.append("   and c.beenDeleted=? ");
	...
	public List<CloudFunctionAndroidDto> getFunctionsByUserId(List<String> userId_list) {
		if (CollectionUtils.isEmpty(userId_list)) {
			return Lists.newArrayList();
		}
		List<String> _userId_list = Lists.newArrayList();

		for (String userId : userId_list) {
			if (StringUtil.isNullOrEmpty(userId)) {
				continue;
			}
			if (_userId_list.contains(userId)) {
				continue;
			}
			_userId_list.add(userId);
		}
		if (CollectionUtils.isEmpty(_userId_list)) {
			return Lists.newArrayList();
		}
		List<Object> args = Lists.newArrayList();
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT  a.userId,c.id,c.code ");
		sql.append(" from cloud_user_role a ");
		sql.append(" join cloud_function_role b on a.roleId=b.roleId");
		sql.append(" join cloud_function c on b.functionId=c.id");
		sql.append(" where a.userId in  ('" + StringUtils.join(_userId_list, "','") + "') ");
		sql.append("   and a.beenDeleted=? ");
		sql.append("   and b.beenDeleted=? ");
		sql.append("   and c.beenDeleted=? ");
		args.add(BakDeleteModel.NO_DELETED);
	...
	public List<CloudFunctionAndroidDto> getTenantFunctionsByUserId(List<String> userId_list) {
		if (CollectionUtils.isEmpty(userId_list)) {
			return Lists.newArrayList();
		}
		List<String> _userId_list = Lists.newArrayList();
		...
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT  a.userId,c.id,c.code ");
		sql.append(" from cloud_tenant_user_role a ");
		sql.append(" left join cloud_tenant_function_role b on a.roleId=b.roleId");
		sql.append(" left join cloud_function c on b.functionId=c.id");
		sql.append(" where a.userId in  ('" + StringUtils.join(_userId_list, "','") + "') ");
		sql.append("   and a.beenDeleted=? ");
		sql.append("   and b.beenDeleted=? ");
		sql.append("   and c.beenDeleted=? ");
