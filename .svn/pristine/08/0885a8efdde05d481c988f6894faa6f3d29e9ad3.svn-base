...
public class TenantRestNpController {
	...
	public RestResultDto<List<SystemListDto>> getSystemList(HttpServletResponse response) {
		try {
			Map<String, String> paramMap = new JsonMapper().fromJson(SpringmvcUtils.getParameter(ManagementConstant.REST_PMS), Map.class);
			return RestResultDto.newSuccess(this.groupCompanyUserService.getSystemList(paramMap.get("userId")), "查询成功");
		} catch (Exception e) {
			return RestResultDto.newFalid("TenantRestNpController.getSystemList:查询失败", e.getMessage());
		}
	}
	...
	public RestResultDto<TenantDto> getById(HttpServletResponse response) {
		try {
			Map<String, String> paramMap = new JsonMapper().fromJson(SpringmvcUtils.getParameter(ManagementConstant.REST_PMS), Map.class);
			return RestResultDto.newSuccess(this.tenantService.getById(SpringmvcUtils.getParameter("id")), "查询成功");
		} catch (Exception e) {
			return RestResultDto.newFalid("TenantRestNpController.getById:查询租户信息失败", e.getMessage());
		}
	}
}
