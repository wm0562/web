...
public class WorkElementDaoImpl extends SimpleHibernateRepository<WorkElement, String> implements IWorkElementDao {
	...
	public List<WorkElementPageDto> syncWeByPage(String tenantId, long syncTime, Integer pageSize, Integer pageNumber) {
		...
		sql.append(
				" SELECT bwe.id as id, bwe.f_name as name, bwe.f_shape as shape, f_deptId  as departmentId, f_params_done  as paramsDone, f_radius as radius, bwe.beenDeleted as beenDeleted,   (case  when (UNIX_TIMESTAMP(bwe.deletedTime)*1000> ? and bwe.beenDeleted = ? and UNIX_TIMESTAMP(bwe.createTime)<= ?) then 3  ");
	...
	public Page<WorkElement> syncWorkElementsByPage(Pageable pageable, Map<String, Object> paramMap) {
		...
		if (null != syncTime && ZERO_LONG != syncTime) {
			...
			// = ? ) ");
			// sql.append(
			// " or (UNIX_TIMESTAMP(bwe.lastChangeTime)*1000> ? and bwe.beenDeleted = ? and
			// UNIX_TIMESTAMP(bwe.createTime)<= ? ) ");
			// sql.append(
			// " or (UNIX_TIMESTAMP(bwe.deletedTime)*1000> ? and bwe.beenDeleted = ? and
			// UNIX_TIMESTAMP(bwe.createTime)<= ?)) ");
			// args.add(syncTime);
			// args.add(BakDeleteModel.NO_DELETED);
			// args.add(syncTime);
			...
			sql.append(" AND UNIX_TIMESTAMP(bwe.lastChangeTime)*1000>= ? ");
			args.add(syncTime);
		}
		// 得到总记录数
		...
		List<WorkElement> pageList = jdbcTemplate.query(sqlString, args.toArray(), new WorkElementRowMapper());
	...
	public List<IdCodeNameDto> getWorkElementsByCodes(String tenantId, List<String> codes) {
		if (StringUtils.isEmpty(tenantId) || CollectionUtils.isEmpty(codes)) {
			return null;
		}
		...
		StringBuffer sql = new StringBuffer();
		sql.append(" select a.id,a.f_name name,a.f_code code ");
		sql.append(" from cloud_work_element a ");
		sql.append(" where a.f_tenantId=? ");
		sql.append("   and a.beenDeleted=0 ");
		sql.append("   and a.f_code in (" + inStr + ") ");

		List<Object> args = Lists.newArrayList();
		args.add(tenantId);
		args.addAll(codes);

		return jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(IdCodeNameDto.class));
	}
}
