...
public class CloudStaffUserTempServiceImpl implements ICloudStaffUserTempService {
	...
	private static final String DEL_PWD = "123456"; // 默认密码123456
	...
	@Override
	public Map<String, Object> importData(CloudStaffUserTemp temp, String tenantId, String marks, int row) throws Exception {
		...
		if (StringUtils.isNotEmpty(temp.getJoinWorkTime())) {
			try {
				if (!validateDate(temp.getJoinWorkTime())) {
					...
					succFlag = false;
				} else if (StringUtils.isNotBlank(model.getBirthday()) && DateUtil.isBefore(DateUtil.parse(model.getBirthday(), DateUtil.DATE_FORMAT), DateUtil.parse(temp.getJoinWorkTime(), DateUtil.DATE_FORMAT))) {
		...
		if (StringUtils.isNotEmpty(temp.getIsLeave())) {
			if (temp.getIsLeave().equals(IsLeaveEnum.ZAIZHI.getValue())) {
				model.setIsLeave(IsLeaveEnum.ZAIZHI.getKey());
			} else if (temp.getIsLeave().equals(IsLeaveEnum.LIZHI.getValue())) {
				model.setIsLeave(IsLeaveEnum.LIZHI.getKey());
			} else if (temp.getIsLeave().equals(IsLeaveEnum.TUIXIU.getValue())) {
				model.setIsLeave(IsLeaveEnum.TUIXIU.getKey());
			} else {
				message.append("离职退休标志不对；");
				succFlag = false;
			}
		}
		// 验证离职日期
		if (StringUtils.isNotEmpty(temp.getLeaveTime())) {
			try {
				if (!validateDate(temp.getLeaveTime())) {
					message.append("退休时间日期格式不对；");
					succFlag = false;
				} else if (StringUtils.isNotBlank(model.getJoinWorkTime()) && DateUtil.isBefore(DateUtil.parse(model.getJoinWorkTime(), DateUtil.DATE_FORMAT), DateUtil.parse(temp.getLeaveTime(), DateUtil.DATE_FORMAT))) {
		...
		if (StringUtils.isNotEmpty(temp.getWorkTypeName())) {
			 ...
			 * (StringUtil.equals(WorkTypeEnum.CLEAN.getValue(), temp.getWorkTypeName())) {
		...
		if (StringUtils.isNotEmpty(temp.getPhone())) {
		...
		if (StringUtils.isEmpty(temp.getUserName())) {
			message.append("用户名不得为空；");
			succFlag = false;
		} else if (StringUtils.isNotEmpty(temp.getUserName()) && cloudUserService.isNameExisted(temp.getUserName(), null)) {
			message.append("用户名已经存在；");
			succFlag = false;
		}

		if (succFlag) {
			message.insert(0, "第" + row + "行：成功！");
			model = cloudStaffService.save(model);

			if (StringUtils.isNotEmpty(temp.getUserName())) {
				// 同步保存user
				CloudUser user = new CloudUser();
				user.setStaffId(model.getId()); // 人员基本信息id
				user.setMobilePushMsgId(null); // 手机推送id
				user.setRongLianAccount(null); // 容联帐号
				user.setUserName(temp.getUserName()); // 用户名
				user.setPassword(MD5.getMD5(DEL_PWD)); // 密码
				user.setBeenSsoLogin(true); // 是否可以单点登录
				user.setLastSsoLoginTime(null); // 最后登录时间
				user.setIsRoot(CloudUser.IS_ROOT_NO); // 是否超级管理员
				user.setPhotoId(null); // 头像id
				user.setPermissionScope(PermissionScopeEnum.SELF_AND_DOWN.getKey());// 权限范围对应PermissionScopeEnum的值，默认本级及以下
				user.setCustomScope(null);// 自定义范围 depart，orgId 用,分割
				user.setImToken(null);// 融云imToken
		...
		// 存入redis
		centralCacheRedisService.putObject(ManagementConstant.MARK_KEY_PREFIX + tenantId, marks);
	...
	@Override
	public Map<String, Object> importDataWithParam(CloudStaffUserTemp tempModel, String tenantId, String marks, int row, Map<String, Object> personalParams) throws Exception {
