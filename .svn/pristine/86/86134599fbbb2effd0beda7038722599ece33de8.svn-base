...
public class CloudUserDaoImpl extends SimpleHibernateRepository<CloudUser, String> implements ICloudUserDao {
	...
	public CloudUser getUserByUserName(String tenantId, String userName) {
		...
		List<CloudUser> cloudUsers = jdbcTemplate.query(sql.toString(), BeanPropertyRowMapper.newInstance(CloudUser.class));
	...
	public CloudUser getUserByIdAndName(String userId, String userName) {
		...
		resultList = jdbcTemplate.query(sql.toString(), argList.toArray(), BeanPropertyRowMapper.newInstance(CloudUser.class));
	...
	public List<UmsLoginReturnInfoDto> getLoginInfo(String tenantCode, String systemCode, String userName) {
		...
		List<UmsLoginReturnInfoDto> rst = null;
	 ...
	 * @return
	 */
	private List<UmsLoginReturnInfoDto> getLoginInfo(String tenantCode, String userName) {
		StringBuffer sql = new StringBuffer();
		...
		sql.append("       	u.staffId,                                                                                   ");
		sql.append("       	u. PASSWORD,                                                                                 ");
		sql.append("       	u.photoId,                                                                                   ");
		sql.append("       	u.rongLianAccount,                                                                           ");
		sql.append("       	u.imToken,			                                                                         ");
		sql.append("       	s. NAME,                                                                                     ");
		sql.append("       	s.phone,                                                                                     ");
		sql.append("       	s.email,                                                                                     ");
		...
		sql.append("       	t.latitudeDone,                                                                              ");
		sql.append("       	t.longitude,                                                                                 ");
		sql.append("       	t.longitudeDone                                                                              ");
		sql.append("       FROM                                                                                          ");
		sql.append("       	cloud_user u                                                                                 ");
		sql.append("       LEFT JOIN cloud_staff s ON s.id = u.staffId AND s.beenDeleted = ?                             ");
		sql.append("       LEFT JOIN cloud_department dp ON dp.id = s.departmentId                                       ");
		sql.append("       LEFT JOIN cloud_organization org ON org.id = s.orgId LEFT JOIN                                ");
		sql.append("        cloud_management_tenant t ON s.tenantId = t.id AND t.beenDeleted = ?                         ");
		sql.append("       WHERE                                                                                         ");
		sql.append("        (                                                                                            ");
		sql.append("       	u.userName = ? and u.beenDeleted=?                                                           ");
		sql.append("       	OR (                                                                                         ");
		sql.append("       		s.phone = ?                                                                              ");
		sql.append("       		AND u.userName IS NOT NULL and u.beenDeleted=?                                           ");
		sql.append("       	)                                                                                            ");
		sql.append("       )                                                                                             ");
		...
		return jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(UmsLoginReturnInfoDto.class));
	}

	private List<UmsLoginReturnInfoDto> getLoginInfoByTenantCode(String tenantCode, String userName) {
		StringBuffer sql = new StringBuffer();
		List<Object> args = new ArrayList<>();
		...
		sql.append(" 	cloud_staff s left join cloud_department dp on dp.id=s.departmentId left join cloud_organization org on org.id=s.orgId ,cloud_management_tenant t					");
	...
	private List<UmsLoginReturnInfoDto> getLoginInfoBySystemCode(String systemCode, String userName) {
		StringBuffer sql = new StringBuffer();
		List<Object> args = new ArrayList<>();

		sql.append(" SELECT u.userName,u.id userId,u.staffId,u.password, u.photoId,s.name,s.phone,s.email,s.tenantId,s.departmentId,s.orgId, ");
		sql.append(" 	dp.depName departmentName, ");
		sql.append(" 	org.orgName, ");
		sql.append("    tnt.latitude,tnt.latitudeDone,tnt.longitude,tnt.longitudeDone ");
		sql.append(" from cloud_user u,cloud_staff s LEFT JOIN cloud_management_tenant tnt on tnt.id=s.tenantId left join cloud_department dp on dp.id=s.departmentId left join cloud_organization org on org.id=s.orgId");
		...
		sql.append(" 		OR ");
		sql.append(" 		EXISTS(select 1 from cloud_user_role ur,cloud_role r,cloud_system t where u.id=ur.userId and ur.roleId=r.id and r.systemId=t.id and t.systemCode=?) ");
		sql.append(" 	) ");
		sql.append("   AND s.beenDeleted=? ");
		sql.append("   AND u.beenDeleted=? ");
	...
	public CloudUserRestDto getUserByUserNameAndTenantCode(String userName, String tenantCode) {
		...
		List<CloudUserRestDto> cloudUserList = jdbcTemplate.query(sql.toString(), new Object[0], BeanPropertyRowMapper.newInstance(CloudUserRestDto.class));
	...
	public List<UserFunctionDto> getFunctionsByUserId(String userId) {
		...
		return jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(UserFunctionDto.class));
	}

	@Override
	public List<UmsLoginReturnInfoDto> getLoginInfoBySystemCode(String userName) {
		StringBuffer sql = new StringBuffer();
		List<Object> args = new ArrayList<>();
		sql.append("  SELECT                                       ");
	...
	public List<CloudUserDto> getUsersByCondiction(Map<String, String> paramMap) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("  SELECT                                                                 ");
		sql.append("  	cu.*, cs.id staffId,cs.name staffName, cs.orgId,cs.departmentId,                                                 ");
		sql.append("  	cu.userName ,   cs.orgName ,	cd.depName departName ,                                                  ");
		sql.append("   cs.tenantId,                          ");
		sql.append("   	cs. CODE,                            ");
		...
		List<CloudUserDto> cloudStaffDtos = jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(CloudUserDto.class));
	...
	public List<CloudUserDto> findListByCompanyIds(List<String> companyIds) {
		...
		return jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(CloudUserDto.class));
	}
	...
	public Page<CloudUserDto> findPageListBySearchDto(Pageable pageable, CloudStaffSearchDto searchDto) {
		...
		List<CloudUserDto> pageList = jdbcTemplate.query(sqlString, argList.toArray(), BeanPropertyRowMapper.newInstance(CloudUserDto.class));
	...
	private Map<String, Object> getSqlOfPageBySearchDto(CloudStaffSearchDto searchDto) {
		...
		sql.append("        JOIN cloud_user u ON s.id = u.staffId                                                      ");
		sql.append("       AND u.beenDeleted = ?                                                                           ");
		sql.append("       WHERE                                                                                           ");
		sql.append("       	s.tenantId =?                                                ");
		sql.append("       AND s.beenDeleted = ?                                                                          ");
	...
	public String getTenantIdByUserId(String userId) throws Exception {
		...
		StringBuffer sql = new StringBuffer();
		sql.append(" select b.tenantId ");
		sql.append(" from cloud_user a,cloud_staff b ");
		sql.append(" where a.staffId=b.id ");
		sql.append("   and a.id=? ");
		List<Object> args = new ArrayList<>();
		args.add(userId);
	...
	public String getAccountByUserId(String userId) throws Exception {
		...
		String sql = "select username from cloud_user where id=?";
		List<Object> args = new ArrayList<>();
		args.add(userId);
	...
	public CloudUser getByAccount(String account) throws Exception {
		...
		String sql = "select * from cloud_user a where a.userName=? ORDER BY createTime DESC";
		List<Object> args = new ArrayList<>();
		args.add(account);

		List<CloudUser> list = jdbcTemplate.query(sql, args.toArray(), BeanPropertyRowMapper.newInstance(CloudUser.class));
	...
	public List<String> getByRoleCodeDivisionId(String roleCode, String divisionId) throws Exception {
		if (StringUtils.isEmpty(roleCode) || StringUtils.isEmpty(divisionId)) {
			return Lists.newArrayList();
		}

		StringBuffer sql = new StringBuffer();
		sql.append("SELECT DISTINCT b.id ");
		sql.append(" from cloud_staff a,cloud_user b,cloud_user_role c,cloud_role d ");
		sql.append(" where a.id=b.staffId ");
		sql.append(" and b.id=c.userId ");
		sql.append(" and c.roleId=d.id ");
		sql.append(" and d.code=? ");
		sql.append(" and a.beenDeleted=0 ");
		sql.append(" and b.beenDeleted=0 ");
		sql.append(" and c.beenDeleted=0 ");
		sql.append(" and d.beenDeleted=0 and ");
		sql.append(" ( EXISTS(select 1 from cloud_organization org where org.divisionId=? and a.orgId=org.id and org.beenDeleted=0)");
		sql.append(" OR ");
		sql.append(" EXISTS(select 1 from cloud_department dep where dep.divisionId=? and a.departmentId=dep.id and (a.orgId is null or a.orgId='') and dep.beenDeleted=0)) ");
	...
	public CloudUser getByPushId(String pushId) {
		...
		filterList.add(new SearchFilter("mobilePushMsgId", Operator.EQ, pushId));
		List<CloudUser> list = findListByFilter(filterList, null);
	...
	public List<UserStaffDto> listUserInfoByTenanIdAndRoleCode(String tenantId, String roleCode) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append(" select a.tenantId,a.id staffId,a.name staffName,b.id userId ");
		sql.append(" from cloud_staff a,cloud_user b ");
		sql.append(" where a.id=b.staffId ");
		...
		sql.append("   and a.beenDeleted=0 ");
		sql.append("   and b.beenDeleted=0 ");
		sql.append("   and EXISTS (select 1 from cloud_user_role ur,cloud_role r where b.id=ur.userid and ur.roleid=r.id and r.code=? and ur.beenDeleted=0 and r.beenDeleted=0) ");
		...
		return jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(UserStaffDto.class));
	}
}
