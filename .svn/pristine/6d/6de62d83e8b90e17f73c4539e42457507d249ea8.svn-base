...
public class QueryUtil {
	...
	private static String createQuerySql(final String sql, final Object... values) {
		if (values != null) {
			...
			for (int i = 0; i < c.length; i++) {
				if (c[i] == '?') {
					s.append(sql.substring(flag, i)).append(values[count].toString());
					flag = i;
					count++;
					flag++;
				}
	...
	public static String getSqlOr(String property, String value, Class<?> type) {
		...
		for (String string : strs) {
			if (!StringUtil.isNullOrEmpty(string)) {
				if ("Integer".equals(typeName) || "Long".equals(typeName) || "Double".equals(typeName) || "Float".equals(typeName) || "Boolean".equals(typeName) || "Date".equals(typeName)) {
	...
	public static String getSqlOr(String property, Collection<?> value) {
		...
		for (Object string : value) {
			if (string instanceof Integer || string instanceof Long || string instanceof Double || string instanceof Float || string instanceof Boolean) {
				sb.append(property).append(" = ").append(string).append(" or ");
	...
	public static String getSqlAnd(String property, Collection<?> value) {
		...
		for (Object string : value) {
			if (string instanceof Integer || string instanceof Long || string instanceof Double || string instanceof Float || string instanceof Boolean) {
				sb.append(property).append(" != ").append(string).append(" and ");
	 ...
	 * @return
	 */
	public static String getPagingSql(String sql, Integer startRow, Integer endRow, String type) {
		StringBuffer pSql = new StringBuffer();
		logger.error("getPagingSql开始组合sql：sql=" + sql + ",startRow=" + startRow + ",endRow=" + endRow + ",type=" + type);
		if (StringUtil.isNullOrEmpty(type)) {
			type = PAGEING_MYSQL;
		}
		if (type.equals(PAGEING_MYSQL)) {
			pSql.append(sql).append(" LIMIT " + (endRow - startRow)).append(" OFFSET " + startRow);
		} else if (type.equals(PAGEING_ORACLE)) {
			pSql.append("select * from ( ").append(" select r.*, ROWNUM RN from ( ").append(" select  x.*, ROWNUM Rmin from(").append(sql).append(" ) x ) r WHERE Rmin <= ").append(endRow).append(" ) WHERE RN > ").append(startRow);
		}
	 ...
	 * @return
	 */
	public static String converSelectSqlToCountHql(String selectSql, String userName) {
		int endIndex = selectSql.indexOf("from");
		String replaced = selectSql.substring(0, endIndex);
		String countSql = selectSql.replace(replaced, "select count(distinct " + userName + ") ");
		return countSql;
	}
