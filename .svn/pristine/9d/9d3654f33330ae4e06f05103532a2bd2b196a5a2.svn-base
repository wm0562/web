...
public class TenantCloudSystemServiceImpl extends
		SimplePagingAndSortingService<TenantSystemRelation, String> implements
		ITenantCloudSystemService {
	private Logger logger = LoggerFactory
			.getLogger(TenantBusinessServiceImpl.class);
	@Resource
	...
	@Override
	public Page<TenantSystemRelationDto> getPageOfCloudSys(Pageable pageable,
			CloudSysSearchDto searchDto) {
		List<SearchFilter> sysFilter = new ArrayList<SearchFilter>();

		String systemName = searchDto.getSystemName();
		if (!StringUtils.isBlank(systemName)) {
			sysFilter.add(new SearchFilter("cloudSystem.systemName",
					Operator.LIKE, systemName));
		}
		if (StringUtils.isEmpty(searchDto.getTenantId())) {
			logger.error("租户id不能为空");
			throw new ServiceException("租户id不能为空");
		}
		sysFilter.add(new SearchFilter("systemType", Operator.EQ,
				CloudSystem.SYSTEM_TYPE_CLOUD));
		// 查询分页
		Page<CloudSystem> page = cloudSystemDao.findPageByFilter(pageable,
				sysFilter);
	...
	private void setCloudSysRealtion(TenantSystemRelationDto relationDto,
			String tenantId, String sysId) {
		List<SearchFilter> relationFilter = Lists.newArrayList();
		relationFilter.add(new SearchFilter("tenantSystemRelation.tenantId",
				Operator.EQ, tenantId));
		relationFilter.add(new SearchFilter(
				"tenantSystemRelation.cloudSystemId", Operator.EQ, sysId));

		List<TenantSystemRelation> relationList = tenantSystemRelationDao
				.findListByFilter(relationFilter, null);
	...
	public void enableCloudSystem(String tenantId, String systemId) {
		...
		relationFilter.add(new SearchFilter("tenantSystemRelation.tenantId",
				Operator.EQ, tenantId));
		relationFilter.add(new SearchFilter(
				"tenantSystemRelation.cloudSystemId", Operator.EQ, systemId));
	...
	public boolean isTenantOpenSystem(String tenantId, String cloudSystemCode) {
		return tenantSystemRelationDao.isTenantOpenSystem(tenantId,
				cloudSystemCode);
	}
}
