...
import static com.vortex.cloud.ums.util.model.Basic.*;
...
public class AreaOrLineAlarmImpl implements AreaOrLineAlarm {
	 ...
	 * @说明：如果在内部为负，在外部为正【好像】
	 */
	public double overMeter(Position position, WorkElement element)
			throws Exception {
		...
		if (shape.equals(Constants.ShapeType.POLYGON)) {
			...
			/*APoint a = allPoint.get(0);
			APoint b = allPoint.get(1);
			double x = getDistance(a, b);// 区域的半径
			double y = getDistance(aPoint, a);// 点到圆心的距离
			return (y - x);// 点到圆的边的距离*/	
			APoint a = allPoint.get(0);
			return getDistance(aPoint, a) - element.getRadius();
		} else if (shape.equals(Constants.ShapeType.POINT)) {
			if (element.getTransferPoints().size() != 1) {
				throw new Exception("点图元必须1个点");
			}
			return getDistance(aPoint, allPoint.get(0));
		} else {
	 ...
	 * @说明：主要还是圆形比较特殊
	 */
	public boolean hasOverlapBetweenElement(WorkElement e1, WorkElement e2)
			throws Exception {
	...
	private boolean hasOverlapBetweenCircleAndCircle(WorkElement circle, WorkElement otherCircle) throws Exception {
		...
		double radius = getDistance(center, circumferencePoint);
		/*另一个圆的信息*/
		...
		double otherRadius = getDistance(otherCenter, otherCircumferencePoint);

		/*判断两个圆是否相交*/
		//计算两个圆心的距离
		double distance = getDistance(center, otherCenter);
	...
	private boolean hasOverlapBetweenCircleAndElement(WorkElement circle,WorkElement element) throws Exception{
		...
		for (int i = 0; i < polygonApointList.size(); i++) {
			...
			// 计算圆心到多边形每条边的距离
			double distance = distanceToSegment(center, S2);
			if (distance < radius) {
				return true;
			}
		}
...
//			Position position2 = (Position) deflecttorService.disposeOne(position);
...
//		}
		return CoordinateConversion.BLToGauss(position.getLongitudeDone(),
	...
	private APoint conversionPointToAPoint(BasePoint point) {
		return CoordinateConversion.BLToGauss(point.getLongitude(), point
				.getLatitude());
	}
}
