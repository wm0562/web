...
public class FileOperateUtil {
	...
	public static List<Map<String, Object>> upload(HttpServletRequest request, String[] params, Map<String, Object[]> values) throws Exception {
		...
		String uploadDir = request.getSession().getServletContext().getRealPath("/") + File.separator + FileOperateUtil.UPLOADDIR;
	...
	public static void download(HttpServletRequest request, HttpServletResponse response, String storeName, String contentType, String realName) throws Exception {
		...
		String ctxPath = request.getSession().getServletContext().getRealPath("/") + File.separator + FileOperateUtil.UPLOADDIR;
	...
	public static <T> void exportExcel(HttpServletRequest request, HttpServletResponse response, String title, String columnFields, String columnNames, List<T> children) throws Exception {
	...
	public static void exportExcelFromMap(HttpServletRequest request, HttpServletResponse response, String title, String columnFields, String columnNames, List<Map<String, String>> children) throws Exception {
		if (StringUtils.isNotEmpty(columnFields)) {
			if (children != null && children.size() > 0) {
				String[] columnField = columnFields.split(",");
				String[] columnName = columnNames.split(",");
				...
				for (Map<String, String> child : children) {
					...
					for (int j = 1; j <= columnField.length; j++) {
						...
						// jxl.write.NumberFormat nf = new jxl.write.NumberFormat("0.00");
						// WritableCellFormat wcfF = new WritableCellFormat(nf);
						// sheet.addCell(new jxl.write.Number(j, i, obj == null ? 0 :
						// Double.valueOf(obj.toString()),wcfF));
						// } else {
						sheet.addCell(new Label(j, i, obj == null ? "" : obj.toString()));
						// }
	 ...
	 *            列对应的字段
	 * @param colNamesArr
	 *            列对应的中文名称(即显示在excel中的)
	 * @param dataList
	 *            填充数据
	 * @throws Exception
	 */
	public static <T> void exportExcel(HttpServletRequest request, HttpServletResponse response, String title, String[] colFieldsArr, String[] colNamesArr, int[] colWidthArr, List<T> dataList) throws Exception {
