...
public class RedisSyncServiceImpl implements IRedisSyncService {
	...
	public void syncUserAuthorityByTenant(List<String> tenantIds) {
		...
		Map<String, Map<String, String>> tenant_user_function_map = null;
		Integer log_userNum = 0;
		// Map<String, Integer> set_map = getTenantParamSets(tenantId_list);
		// if (null == set_map) {
		// set_map = Maps.newHashMap();
		// }
		for (String tenantId : tenantId_list) {
			...
			// tenant_user_function_map =
			// getUserFunctionMap(set_map.get(tenantId), userId_list);
			tenant_user_function_map = getUserFunctionMap(userId_list);
			for (String userId : userId_list) {
				// 获取一个用户的所有功能码及id(key:id,value:code)
				user_function_map = tenant_user_function_map.get(userId);
	...
	public void syncDeptOrgByTenant(List<String> tenantIds) {
		...
		long t_dept = System.currentTimeMillis() - t1;
		t1 = System.currentTimeMillis();
		syncTotalDeptOrg(total_list);
		long t_sync = System.currentTimeMillis() - t1;
		logger.error(String.format("[同步redis,完整机构部门]，总耗时：%sms，存入redis%sms，租户数量：%s，机构部门数量：%s，获取租户耗时：%sms，获取部门耗时：%sms",
	...
	public void syncStaffByTenant(List<String> tenantIds) {
		...
		long t_staff = System.currentTimeMillis() - t1;
		t1 = System.currentTimeMillis();
		syncTotalStaff(total_list);
		long t_sync = System.currentTimeMillis() - t1;
		logger.error(String.format("[同步redis,完整人员]，总耗时：%sms，存入redis%sms，租户数量：%s，人员数量：%s，获取租户耗时：%sms，获取人员耗时：%sms",
				(System.currentTimeMillis() - t0), t_sync, tenantId_list.size(), total_list.size(), t_tenantId,
				t_staff));
	}
	 ...
	 */
	private Map<String, Map<String, String>> getUserFunctionMap(Integer roleStyle, List<String> userId_list) {
		Map<String, Map<String, String>> returnValue = Maps.newHashMap();
		List<CloudFunctionAndroidDto> list = Lists.newArrayList();
		...
		switch (roleStyle) {
		case 0:// 获取用户在租户下系统下角色下功能
			list.addAll(cloudFunctionDao.getFunctionsByUserId(userId_list));
			break;
		case 1:// 获取用户在租户下角色下功能
			list.addAll(cloudFunctionDao.getTenantFunctionsByUserId(userId_list));
			break;
		case 2:// 2种都包含
			list.addAll(cloudFunctionDao.getFunctionsByUserId(userId_list));
			list.addAll(cloudFunctionDao.getTenantFunctionsByUserId(userId_list));
		...
		Map<String, String> one_user = null;
		if (CollectionUtils.isEmpty(list)) {
			return returnValue;
		}
		for (CloudFunctionAndroidDto entity : list) {
			if (StringUtil.isNullOrEmpty(entity.getUserId())) {
				continue;
			}
			if (returnValue.containsKey(entity.getUserId())) {
				one_user = returnValue.get(entity.getUserId());
			} else {
				one_user = Maps.newHashMap();
			}
			one_user.put(entity.getId(), entity.getCode());
			returnValue.put(entity.getUserId(), one_user);
		}
		return returnValue;
	}

	private Map<String, Map<String, String>> getUserFunctionMap(List<String> userId_list) {
		Map<String, Map<String, String>> returnValue = Maps.newHashMap();
		List<CloudFunctionAndroidDto> list = Lists.newArrayList();
		// 获取用户在租户下系统下角色下功能
		list.addAll(cloudFunctionDao.getFunctionsByUserId(userId_list));
