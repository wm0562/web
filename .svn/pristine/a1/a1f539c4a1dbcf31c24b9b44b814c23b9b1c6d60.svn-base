...
public class TenantDivisionDaoImpl extends SimpleHibernateRepository<TenantDivision, String> implements ITenantDivisionDao {
	...
	public List<TenantDivisionDto> listDivisionByOrgIds(List<String> orgids) throws Exception {
		...
		sql.append(" SELECT a.* ");
		sql.append(" from cloud_tenant_division a,cloud_department b ");
		sql.append(" where b.id in (" + str + ") ");
		sql.append("   and a.id=b.divisionId ");
		sql.append("   and b.beenDeleted=0 ");
		sql.append(" UNION ");
		sql.append(" select a.* ");
		sql.append(" from cloud_tenant_division a,cloud_organization b ");
		sql.append(" where b.id in (" + str + ") ");
		sql.append("   and a.id=b.divisionId ");
		sql.append("   and b.beenDeleted=0 ");
		...
		argList.addAll(orgids);
		return jdbcTemplate.query(sql.toString(), argList.toArray(), BeanPropertyRowMapper.newInstance(TenantDivisionDto.class));
	}
}
