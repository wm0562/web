...
public class CloudOrganizationServiceImpl extends SimplePagingAndSortingService<CloudOrganization, String> implements ICloudOrganizationService {
	...
	public CloudOrganization save(CloudOrganizationDto dto) {
		...
		return cloudOrganizationDao.save(org);
	}
	...
	public void update(CloudOrganizationDto dto) {
		...
		if (!old.getOrgName().equals(dto.getOrgName())) {
			jdbcTemplate.execute("UPDATE cloud_staff SET orgName = '" + dto.getOrgName() + "' WHERE orgId = '" + old.getId() + "'");
		}
	...
	public List<Map<String, String>> getDepartmentsOrOrgByCondiction(Map<String, Object> paramMap) {
		...
		if (StringUtils.isBlank(companyType)) {// 不过滤类型
			...
			cloudDepartments = cloudDepartmentDao.findListByFilter(searchFilters, sort);
			organizations = cloudOrganizationDao.findListByFilter(searchFilters, sort);
		} else {
	...
	public TenantDeptOrgDto getDepartmentOrOrgById(String rootDeptId, List<Integer> beenDeletedFlags) {
		...
		// cloudOrganizationDao.findOne(rootDeptId);
		...
		if (organization != null) {
			...
			tenantDeptOrgDto.setDivisionId(organization.getDivisionId());
		} else {
			...
			tenantDeptOrgDto.setDivisionId(dto.getDivisionId());
		}
	...
	public String getMaxNodeCodeByParentId(String tabName, String parentIdColName, String parentId) {
		...
		return this.cloudOrganizationDao.getMaxNodeCodeByParentId(tabName, parentIdColName, parentId);
	}

	@Override
	public String getNextNodeCodeByFirstLevel(String tenantId) {
		// TODO Auto-generated method stub
		return this.cloudOrganizationDao.getNextNodeCodeByFirstLevel(tenantId);
	}
