...
public class CloudOrganizationDaoImpl extends SimpleHibernateRepository<CloudOrganization, String> implements ICloudOrganizationDao {
	...
	public List<TenantDeptOrgDto> findOrganizationChild(String departId, String nodeCode, String id, List<Integer> beenDeletedFlags) {
		...
		sql.append("            	'3' type,'org' companyType,                    ");
		sql.append("            	co.id,                                         ");
		...
		sql.append("            	co.email  , co.beenDeleted,co.deletedTime,co.createTime,co.lastChangeTime,co.orderIndex,co.divisionId                                     ");
		...
		sql.append(" ORDER BY co.orderIndex asc  ");
	...
	public List<TenantDeptOrgDto> getDepartmentsOrOrgByIds(String[] ids, List<Integer> beenDeletedFlags) {
		...
		sql.append("                     	cd.email, cd.beenDeleted,cd.deletedTime,cd.createTime,cd.lastChangeTime,cd.orderIndex,cd.divisionId                               ");
		sql.append("                     FROM                                      ");
	...
	public String getMaxNodeCodeByParentId(String tabName, String parentIdColName, String parentId) {
		StringBuffer sql = new StringBuffer();
		sql.append(" select  CONCAT(left(nodecode,LENGTH(nodecode)-2) , right((right(nodecode,2)+101),2)) ");
		sql.append(" from " + tabName + " ");
		sql.append(" where nodeCode+0=(select max(nodeCode+0) from " + tabName + " where " + parentIdColName + "='" + parentId + "') ");
		List<String> rst = jdbcTemplate.queryForList(sql.toString(), String.class);
	...
	public String getNextNodeCodeByFirstLevel(String tenantId) {
		StringBuffer sql = new StringBuffer();
		sql.append(" select  CONCAT(left(nodecode,LENGTH(nodecode)-2) , right((right(nodecode,2)+101),2)) ");
		sql.append(" from cloud_organization ");
		sql.append(" where nodeCode+0=(select max(nodeCode+0) from cloud_organization where tenantId ='" + tenantId + "' and LENGTH(nodecode)=2) ");
		List<String> rst = jdbcTemplate.queryForList(sql.toString(), String.class);
		if (CollectionUtils.isEmpty(rst)) {
			return "01";
		} else {
