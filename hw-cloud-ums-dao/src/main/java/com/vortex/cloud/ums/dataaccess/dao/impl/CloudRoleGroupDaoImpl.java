...
public class CloudRoleGroupDaoImpl extends SimpleHibernateRepository<CloudRoleGroup, String> implements ICloudRoleGroupDao {
	...
	@Override
	public CloudRoleGroup findByCode(String groupCode, String systemId) {
		List<SearchFilter> filterList = new ArrayList<SearchFilter>();
		filterList.add(new SearchFilter("code", Operator.EQ, groupCode));
		filterList.add(new SearchFilter("systemId", Operator.EQ, systemId));
		List<CloudRoleGroup> list = findListByFilter(filterList, null);
