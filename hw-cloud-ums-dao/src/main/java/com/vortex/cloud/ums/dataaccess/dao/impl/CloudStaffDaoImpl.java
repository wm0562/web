...
public class CloudStaffDaoImpl extends SimpleHibernateRepository<CloudStaff, String> implements ICloudStaffDao {
	...
	public CloudStaff save(CloudStaff entity) {
		...
		// 更新人员缓存
		if (null == _entity.getBeenDeleted() || BakDeleteModel.NO_DELETED == _entity.getBeenDeleted()) {
			updateRedis(_entity);
		}
		return _entity;
	}
	...
	public CloudStaff update(CloudStaff entity) {
		...
		if (null == _entity.getBeenDeleted() || BakDeleteModel.NO_DELETED == _entity.getBeenDeleted()) {
			...
			deleteRedis(_entity.getTenantId(), _entity.getId());
		}
		return _entity;
	}
	...
	private void updateRedis(CloudStaff entity) {
		...
		String redisKey = ManagementConstant.REDIS_PRE_TENANT_STAFFIDS + ManagementConstant.REDIS_SEPARATOR + entity.getTenantId();
		// 获取租户下人员缓存的key
	...
	public Page<CloudStaffDto> findPageBySearchDto(Pageable pageable, CloudStaffSearchDto searchDto) {
		logger.error(searchDto == null ? "插入传入dto为空" : searchDto.toString());
		...
		String sqlString = QueryUtil.getPagingSql(sql.toString(), startRow, endRow, "mysql");

		List<CloudStaffDto> pageList = jdbcTemplate.query(sqlString, argList.toArray(), BeanPropertyRowMapper.newInstance(CloudStaffDto.class));
	...
	private Map<String, Object> getSqlOfPageBySearchDto(CloudStaffSearchDto searchDto) {
		...
		sql.append("        u.userName userName ,s.postName ,  cps.parmName   partyPostName,u.lockuser   ");
		sql.append("   ,u.permissionScope,u.customScope                                                       ");
		sql.append("       FROM                                                                                            ");
		...
		sql.append("       LEFT JOIN cloud_tenantparameter_setting cps ON cps.typeId = cpt.id AND cps.parmCode = s.partyPostId   AND cps.tenantId=s.tenantId     ");
		sql.append("       AND cps.beenDeleted = ?                                                                         ");
		sql.append("                                                                                                       ");
		sql.append("       WHERE                                                                                           ");
		sql.append("       	s.tenantId =?                                                ");
		...
		if (FALSE.equals(ckRange)) {
			...
			List<String> companyIds = searchDto.getCompanyIds();
			if (CollectionUtils.isNotEmpty(companyIds)) {
				if (StringUtils.isNotEmpty(searchDto.getOrgId())) {
					sql.append(" 	AND s.orgId = ? ");
				} else if (StringUtils.isNotEmpty(searchDto.getDepartmentId())) {
					sql.append(" 	AND s.departmentId = ? and (s.orgid is null or s.orgid='') ");
				}
				argList.add(companyIds.get(0));
			}
			...
			if (StringUtils.isBlank(orgId)) {
				...
				// 获取部门下的组织
				List<TenantDeptOrgDto> tenantDeptOrgDtoList = cloudDepartmentDao.findDeptOrgList(searchDto.getTenantId(), departmentId, null);
				...
				List<TenantDeptOrgDto> tenantDeptOrgDtos = cloudOrganizationDao.findOrganizationChild(departId, nodeCode, id, null);
		...
		if (StringUtils.isNotBlank(ageGroupStart)) {
			sql.append(" 	AND ? <= (year(now())-year(s.birthday)-1) + ( DATE_FORMAT(s.birthday, '%m%d') <= DATE_FORMAT(NOW(), '%m%d') )                     ");
	...
	private void addOrder(String staffAli, StringBuffer sql, Sort sort) {
		...
		for (Order order : sort) {
			...
			if (order.getProperty().equalsIgnoreCase("name")) {
				...
				sb.append(" " + staffAli + "." + order.getProperty() + (order.isAscending() ? " ASC " : " DESC ") + ", ");
			}
		}
	...
	public CloudStaff getStaffByCode(String staffCode, String tenantId) {
		...
		CloudStaff cloudStaff = jdbcTemplate.queryForObject(sql.toString(), BeanPropertyRowMapper.newInstance(CloudStaff.class));
		return cloudStaff;
	}
	...
	public List<CloudStaffDto> getStaffsByUserIds(List<String> ids) {
		...
		sql.append(" SELECT cs.*, cu.id as userId,cu.permissionScope,cu.customScope  FROM cloud_staff cs LEFT JOIN cloud_user cu ON cu.staffId = cs.id WHERE cs.beenDeleted = ? AND cu.beenDeleted = ? ");
		args.add(false);
		args.add(false);
		if (CollectionUtils.isNotEmpty(ids)) {
			String param = generateInParam(ids);
		...
		List<CloudStaffDto> CloudStaffDtoList = jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(CloudStaffDto.class));
	...
	public List<CloudStaff> getStaffIdsByNames(List<String> names, String tenantId) {
		...
		List<CloudStaff> list = jdbcTemplate.query(sql.toString(), argList.toArray(new String[argList.size()]), BeanPropertyRowMapper.newInstance(CloudStaff.class));
		return list;
	}
	...
	public List<CloudStaffDto> loadStaffsByFilter(Map<String, Object> paramMap) {
		...
		sql.append("          	u.mobilePushMsgId  ,u.imToken ,u.photoId,u.permissionScope,u.customScope ");
		sql.append("          FROM                                           ");
		sql.append("          	cloud_staff s                                ");
		sql.append("          LEFT JOIN cloud_user u ON s.id = u.staffId     ");
		sql.append("          WHERE                                          ");
		...
		return jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(CloudStaffDto.class));
	}
	...
	public CloudStaffDto getById(String id) {
		...
		sql.append("             	cu.userName userName,cu.permissionScope,cu.customScope,                                         ");
		sql.append("             	cu.photoId,cd.depName companyName,cu.lockuser,                 ");
		sql.append("  	cs.isWillMan,cs.willCheckDivisionIds,cs.willWorkUnit,cs.address,cs.photograph                               ");
		sql.append("             FROM                                                             ");
		sql.append("             	cloud_staff cs                                                ");
		...
		sql.append("              AND cps7.parmCode = cs.maritalStatusId                                              ");
		...
		sql.append("              AND cps8.parmCode = cs.politicalStatusId                                              ");
		...
		sql.append("              AND cps11.parmCode = cs.socialSecuritycase                                              ");
		...
		args.add(id);
		args.add(BakDeleteModel.NO_DELETED);
		List<CloudStaffDto> cloudStaffDtos = jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(CloudStaffDto.class));
	...
	public List<CloudStaffPageDto> syncStaffByPage(String tenantId, long syncTime, Integer pageSize, Integer pageNumber) {
		List<Object> args = Lists.newArrayList();
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT s.id ,u.id as userId, s.name, s.orgId as orgId, s.departmentId as departmentId,  u.imToken, u.photoId, s.beenDeleted, s.orderIndex, s.phone, s.nameInitial, ");
		sql.append(" cps.id as cpId, cps.parmCode, cps.parmName, cpsPost.id AS postionId, cpsPost.parmCode AS postionCode, cpsPost.parmName AS postionName ");
		...
		if (syncTime != ZERO_LONG) {
			...
			sql.append(" AND CASE  1=1 WHEN ((u.id is null) or (s.lastChangeTime >= u.lastChangeTime ) ) THEN  ((UNIX_TIMESTAMP(s.createTime)*1000> ? and s.beenDeleted = ? ) ");
			sql.append(" or (UNIX_TIMESTAMP(s.lastChangeTime)*1000> ?  and s.beenDeleted = ? and UNIX_TIMESTAMP(s.createTime) *1000  <= ?) ");
			sql.append(" or (UNIX_TIMESTAMP(s.deletedTime)*1000> ? and s.beenDeleted = ? and UNIX_TIMESTAMP(s.createTime) *1000 <= ?)) ");

			sql.append(" ELSE   ");
			sql.append("  ((UNIX_TIMESTAMP(u.createTime)*1000> ? and u.beenDeleted = ? ) ");
			sql.append(" or (UNIX_TIMESTAMP(u.lastChangeTime)*1000> ?  and u.beenDeleted = ? and UNIX_TIMESTAMP(u.createTime) *1000  <= ?) ");
			sql.append(" or (UNIX_TIMESTAMP(u.deletedTime)*1000> ? and u.beenDeleted = ? and UNIX_TIMESTAMP(u.createTime) *1000 <= ?)) END ");
		...
		return jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(CloudStaffPageDto.class));
	}
	...
	public List<CloudStaffPageDto> findAllStaffByPage(String tenantId, Integer isDeleted) {
		List<Object> args = Lists.newArrayList();
		StringBuffer sql = new StringBuffer();
		sql.append(" SELECT s.id , s.name, s.orgId as orgId, s.departmentId as departmentId,  u.imToken, u.photoId, s.beenDeleted, s.orderIndex, s.nameInitial, ");
	...
	public List<CloudStaffDto> findListBySearchDto(Sort defSort, CloudStaffSearchDto searchDto) {
		...
		List<CloudStaffDto> pageList = jdbcTemplate.query(sql.toString(), argList.toArray(), BeanPropertyRowMapper.newInstance(CloudStaffDto.class));
	...
	public Page<CloudStaffDto> syncStaffsByPage(Pageable pageable, Map<String, Object> paramMap) {
		...
		String tenantId = (String) paramMap.get("tenantId");
		// 在职状态
		String isLeave = (String) paramMap.get("isLeave");
		Long syncTime = null;
		...
		sql.append("          	u.userName userName,                         ");
		sql.append("          	u.rongLianAccount,                           ");
		sql.append("          	u.mobilePushMsgId  ,u.imToken ,u.photoId,u.permissionScope,u.customScope,t.parmName as workTypeName ");
		sql.append("          FROM                                           ");
		sql.append("          	cloud_staff s                                ");
		sql.append("          LEFT JOIN cloud_user u ON s.id = u.staffId     ");

		sql.append(" left join (select pt.typeCode,p.* from cloud_parameter_type pt,cloud_tenantparameter_setting p where pt.id=p.typeId and p.tenantId=? and pt.typeCode=? and p.beenDeleted = ? and pt.beenDeleted = ?) t ");
		sql.append(" on s.workTypeCode=t.parmCode ");
		...
		if (null != syncTime && ZERO_LONG != syncTime) {
			...
			// sql.append(" AND CASE 1=1 WHEN ((u.id is null) or
			// (s.lastChangeTime >=
			// u.lastChangeTime ) ) THEN ((UNIX_TIMESTAMP(s.createTime)*1000> ?
			// and
			// s.beenDeleted = ? ) ");
			// sql.append(" or (UNIX_TIMESTAMP(s.lastChangeTime)*1000> ? and
			// s.beenDeleted =
			// ? and UNIX_TIMESTAMP(s.createTime) *1000 <= ?) ");
			// sql.append(" or (UNIX_TIMESTAMP(s.deletedTime)*1000> ? and
			// s.beenDeleted = ?
			// and UNIX_TIMESTAMP(s.createTime) *1000 <= ?)) ");
			// sql.append(" ELSE ");
			// sql.append(" ((UNIX_TIMESTAMP(u.createTime)*1000> ? and
			// u.beenDeleted = ? )
			// ");
			// sql.append(" or (UNIX_TIMESTAMP(u.lastChangeTime)*1000> ? and
			// u.beenDeleted =
			// ? and UNIX_TIMESTAMP(u.createTime) *1000 <= ?) ");
			// sql.append(" or (UNIX_TIMESTAMP(u.deletedTime)*1000> ? and
			// u.beenDeleted = ?
			// and UNIX_TIMESTAMP(u.createTime) *1000 <= ?)) END ");
			//
			// args.add(syncTime);
			// args.add(BakDeleteModel.NO_DELETED);
			//
			...
			/**
			 * 20180106应杭向明组需要简化时间匹配条件
			 */
			sql.append(" AND (UNIX_TIMESTAMP(s.lastChangeTime)*1000>= ? OR UNIX_TIMESTAMP(u.lastChangeTime)*1000>= ?) ");
			args.add(syncTime);
			args.add(syncTime);
		}
		// 得到总记录数
		...
		List<CloudStaffDto> pageList = jdbcTemplate.query(sqlString, args.toArray(), BeanPropertyRowMapper.newInstance(CloudStaffDto.class));
		return new PageImpl<>(pageList, pageable, totalCnt);
	}
	...
	public List<CloudStaffDto> getStaffInfoByUserIds(List<String> ids) {
		...
		sql.append("          	cloud_staff s                                ");
		sql.append("          LEFT JOIN cloud_user u ON s.id = u.staffId     ");
		sql.append("          WHERE    s.beenDeleted = ?   and    u.beenDeleted = ?                     ");
		sql.append("    AND      u.id   in ('" + StringUtils.join(ids, "','") + "') ");
		args.add(BakDeleteModel.NO_DELETED);
		args.add(BakDeleteModel.NO_DELETED);
		return jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(CloudStaffDto.class));
	}
	...
	@Override
	public List<CloudStaffDto> getWillManStaffUserList(String tenantId, String name, String willCheckDivisionIds, String ignoreUserIds) {
		if (StringUtil.isNullOrEmpty(tenantId)) {
			return Lists.newArrayList();
		}
		List<Object> args = Lists.newArrayList();
		StringBuffer sql = getWillManFilterSql(tenantId, name, willCheckDivisionIds, ignoreUserIds);
		return jdbcTemplate.query(sql.toString(), args.toArray(), BeanPropertyRowMapper.newInstance(CloudStaffDto.class));
	}

	@Override
	public Page<CloudStaffDto> getWillManStaffUserPage(String tenantId, String name, String willCheckDivisionIds, String ignoreUserIds, Pageable pageable) {
		List<Object> args = Lists.newArrayList();
		StringBuffer sql = getWillManFilterSql(tenantId, name, willCheckDivisionIds, ignoreUserIds);
		// 得到总记录数
		String sqlCnt = " SELECT COUNT(1) FROM ( " + sql.toString() + " ) t ";
	...
	private StringBuffer getWillManFilterSql(String tenantId, String name, String willCheckDivisionId, String ignoreUserIds) {
		StringBuffer sql = new StringBuffer();
		sql.append("          SELECT                                         ");
		sql.append("          	s.*, u.id userId,                            ");
		sql.append("          	u.userName userName                         ");
		...
		sql.append("          	cloud_staff s                                ");
		sql.append("          LEFT JOIN cloud_user u ON s.id = u.staffId     ");
		sql.append("          WHERE    s.beenDeleted = " + BakDeleteModel.NO_DELETED + "                          ");
		sql.append("    AND      u.beenDeleted = " + BakDeleteModel.NO_DELETED + " ");
		sql.append("    AND      s.tenantId = '" + tenantId + "' ");
		sql.append("    AND      s.isWillMan = true ");

		if (!StringUtil.isNullOrEmpty(name)) {
			sql.append("    AND      s.name LIKE '%" + name + "%' ");
		}
		if (!StringUtil.isNullOrEmpty(willCheckDivisionId)) {
			String[] id_arr = StringUtil.splitSymbol(willCheckDivisionId, ",");
			...
			if (CollectionUtils.isNotEmpty(id_list)) {
				sql.append("    AND ( s.willCheckDivisionIds LIKE '%");
				sql.append(StringUtils.join(id_list, "%' or s.willCheckDivisionIds LIKE '%"));
				sql.append("%' )");
			}
		}
		if (!StringUtil.isNullOrEmpty(ignoreUserIds)) {
			String[] id_arr = StringUtil.splitSymbol(ignoreUserIds, ",");
			List<String> id_list = Lists.newArrayList();
			for (String id : id_arr) {
				if (StringUtil.isNullOrEmpty(id)) {
					continue;
			...
			if (CollectionUtils.isNotEmpty(id_list)) {
				...
				sql.append(StringUtils.join(id_list, "','"));
				sql.append("') ");
			}
		}
		return sql;
	...
	public CloudStaffRestDto getStaffByCodeAndTenantCode(String code, String tenantCode) {
		...
		List<CloudStaffRestDto> cloudStaffList = jdbcTemplate.query(sql.toString(), new Object[0], BeanPropertyRowMapper.newInstance(CloudStaffRestDto.class));
	...
	public List<StaffDto> listStaff(String name, String phone, List<String> orgIds, String tenantId, String containManager) throws Exception {
		...
		if (CollectionUtils.isNotEmpty(orgIds)) {
			...
			sql.append(" SELECT t.id staffId,t.`name`,t.`code`,t.phone,case when t.orgId is not null and t.orgId<>'' then t.orgId else t.departmentId end orgId,orgName ");
			sql.append(" from cloud_staff t ");
			sql.append(" where (t.orgId is null and t.departmentId in (" + ids + ")) or (t.orgid is not null and t.orgId in (" + ids + ")) ");
	...
	@Override
	public Long syncStaffCount(String tenantId, Long syncTime) {
