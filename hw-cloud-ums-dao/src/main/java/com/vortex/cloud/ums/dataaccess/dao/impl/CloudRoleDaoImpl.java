...
public class CloudRoleDaoImpl extends SimpleHibernateRepository<CloudRole, String> implements ICloudRoleDao {
	...
	public List<String> getRoleIdsByUserId(String userId) {
		...
		sql.append("   SELECT                               ");
		sql.append("   	distinct r.id                                 ");
		sql.append("   FROM                                 ");
		sql.append("   	cloud_role r, cloud_user_role cur, cloud_user u	");
		sql.append("   WHERE                                ");
		sql.append("   	r.id = cur.roleId  and u.id=cur.userId          		");
		sql.append("   	AND u.id = ?                  ");
		sql.append("   	AND cur.beenDeleted = ?   AND u.beenDeleted = ? AND r.beenDeleted = ? ");
		argList.add(userId);
		argList.add(BakDeleteModel.NO_DELETED);
		argList.add(BakDeleteModel.NO_DELETED);
		argList.add(BakDeleteModel.NO_DELETED);
