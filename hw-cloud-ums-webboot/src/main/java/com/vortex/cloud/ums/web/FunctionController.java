...
public class FunctionController extends BaseController {
	...
	@RequestMapping(value = "loadTreeByTenantId" + FORE_DYNAMIC_SUFFIX, method = RequestMethod.POST)
	public RestResultDto<String> loadTreeByTenantId(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			FunctionGroupTree functionGroupTree = FunctionGroupTree.getInstance();
			functionGroupTree.reloadFunctionTreeByTenantId(tenantId);
			String json = treeService.generateJsonCheckboxTree(functionGroupTree, false);
			return RestResultDto.newSuccess(json);
		} catch (Exception e) {
			logger.error("FunctionController.loadTreeByTenantId", e);
			return RestResultDto.newFalid("加载树失败", e.getMessage());
		}
	}
