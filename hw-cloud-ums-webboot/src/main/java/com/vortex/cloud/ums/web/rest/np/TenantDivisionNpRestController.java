...
public class TenantDivisionNpRestController extends BaseController {
	...
	public RestResultDto loadDivisionTreeByLevel(HttpServletRequest request) {
		String msg = null;
		Integer result = RestResultDto.RESULT_FAIL;
		Object data = null;
		String exception = null;
		...
		try {
			...
			String tenantId = paramMap.get(TENANT_ID);
			Integer level = null;
			if (MapUtils.isNotEmpty(paramMap) && paramMap.containsKey(REQ_PARAM_DIVISION_LEVEL)) {
				level = Integer.valueOf(String.valueOf(paramMap.get(REQ_PARAM_DIVISION_LEVEL)));
			}
			// 注意：divisionId不是必须的请求参数，如果没有传入，则取租户下整颗树

			TenantDivisionTree tree = TenantDivisionTree.getInstance();
			tree.reloadTenantDivisionTreeByLevel(tenantId, level);
			String jsonStr = treeService.generateJsonCheckboxTree(tree, false);
	...
	public RestResultDto<List<TenantDivisionDto>> listByPermission(HttpServletRequest request, CloudUserDto user) {
		try {
			Map<String, String> paramMap = jm.fromJson(SpringmvcUtils.getParameter(ManagementConstant.REST_PMS),
					Map.class);
			return RestResultDto.newSuccess(tenantDivisionService.listByPermission(paramMap.get("userId")), "查询成功");
		} catch (Exception e) {
			logger.error("TenantDivisionNpRestController.listByPermission", e);
