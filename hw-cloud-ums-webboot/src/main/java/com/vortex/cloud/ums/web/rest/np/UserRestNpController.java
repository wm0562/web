...
public class UserRestNpController {
	...
	public RestResultDto loginAndSavePushID(HttpServletRequest request) throws Exception {
		...
		try {
			...
			// 登录
			data = loginService.login(paramMap.get(TEANANT_CODE), paramMap.get(SYSTEM_CODE), paramMap.get(USER_NAME), MD5.getMD5(paramMap.get(PASSWORD)), paramMap.get(MOBILE_PUSH_MSG_ID), paramMap.get(IP));
	 ...
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping(value = "loginByEvidence" + ManagementConstant.PERMISSION_SUFFIX_READ, method = { RequestMethod.GET, RequestMethod.POST })
	public RestResultDto loginByEvidence(HttpServletRequest request) throws Exception {
		String msg = null;
		Integer result = null;
		Object data = null;
		String exception = null;
		...
		try {
			...
			// 登录
			data = loginService.loginByPasswordOrToken(paramMap.get(TEANANT_CODE), paramMap.get(SYSTEM_CODE), paramMap.get(USER_NAME), MD5.getMD5(paramMap.get(PASSWORD)), paramMap.get(IP));
			if (data != null) {
				msg = "成功获取用户信息";
				result = ManagementConstant.REST_RESULT_SUCC;
			} else {
	...
	public RestResultDto resetPassword(HttpServletRequest request) {
		String msg = null;
		Integer result = ManagementConstant.REST_RESULT_SUCC;
		Object data = null;
		String exception = null;
	 ...
	 * @return
	 */
	@RequestMapping(value = "addDtl" + ManagementConstant.PERMISSION_SUFFIX_SA, method = { RequestMethod.GET, RequestMethod.POST })
	public RestResultDto<Boolean> addDtl(HttpServletRequest request, CloudUserDto dto) {
		try {
	...
	@RequestMapping(value = "updateDtl" + ManagementConstant.PERMISSION_SUFFIX_SA, method = { RequestMethod.GET, RequestMethod.POST })
	public RestResultDto<Boolean> updateDtl(HttpServletRequest request, CloudUserDto user) {
		try {
			cloudUserService.update(user);
			return RestResultDto.newSuccess(true, "更新成功");
		} catch (Exception e) {
			logger.error("updateDtl", e);
			return RestResultDto.newFalid("更新失败", e.getMessage());
		}
	}

	@RequestMapping(value = "deleteUser", method = { RequestMethod.GET, RequestMethod.POST })
	public RestResultDto<Boolean> deleteUser(HttpServletRequest request, CloudUserDto user) {
		try {
			Map<String, String> paramMap = jm.fromJson(SpringmvcUtils.getParameter(ManagementConstant.REST_PMS), Map.class);
			cloudUserService.deleteUser(paramMap.get("userId"));
	...
	public RestResultDto<?> getByRoleCodeDivisionId(HttpServletRequest request, CloudUserDto user) {
		try {
			Map<String, String> paramMap = jm.fromJson(SpringmvcUtils.getParameter(ManagementConstant.REST_PMS), Map.class);
			if (MapUtils.isEmpty(paramMap)) {
				throw new VortexException("参数不能为空");
			...
			String divisionId = paramMap.get("divisionId");
			if (StringUtil.isNullOrEmpty(roleCode)) {
				throw new VortexException("角色编号不能为空");
			}
			if (StringUtil.isNullOrEmpty(divisionId)) {
				throw new VortexException("行政区划id不能为空");
			}
			return RestResultDto.newSuccess(cloudUserService.getByRoleCodeDivisionId(roleCode, divisionId), "获取成功");
		} catch (Exception e) {
			logger.error("getByRoleCodeDivisionId", e);
			return RestResultDto.newFalid("获取失败", e.getMessage());
		}
	}

	@RequestMapping(value = "listUserInfoByTenanIdAndRoleCode", method = { RequestMethod.GET, RequestMethod.POST })
	public RestResultDto<List<UserStaffDto>> listUserInfoByTenanIdAndRoleCode(HttpServletRequest request, CloudUserDto user) {
		try {
			Map<String, String> paramMap = jm.fromJson(SpringmvcUtils.getParameter(ManagementConstant.REST_PMS), Map.class);
			return RestResultDto.newSuccess(cloudUserService.listUserInfoByTenanIdAndRoleCode(paramMap.get("tenantId"), paramMap.get("roleCode")), "查询成功");
		} catch (Exception e) {
			logger.error("listUserInfoByTenanIdAndRoleCode", e);
