...
public class CloudSystemController extends BaseController {
	...
	public RestResultDto<DataStore<CloudSystem>> pageList(HttpServletRequest request) {
		try {
			...
			String systemCode = SpringmvcUtils.getParameter("systemCode"); // 页面传入的查询条件，根据系统编号
			String systemName = SpringmvcUtils.getParameter("systemName"); // 页面传入的查询条件，根据系统名称
			if (StringUtils.isNotEmpty(systemCode)) {
				sfList.add(new SearchFilter("systemCode", Operator.LIKE, systemCode));
			}

			if (StringUtils.isNotEmpty(systemName)) {
				sfList.add(new SearchFilter("systemName", Operator.LIKE, systemName));
			}
