...
public class WorkElementRestNpController {
		...
		try {
			...
			for (WorkElement element : list) {
				...
				try {
					...
					if (((SharpTypeEnum.POLYGON.getKey().equalsIgnoreCase(shape) || SharpTypeEnum.CIRCLE.getKey().equalsIgnoreCase(shape) || SharpTypeEnum.RECTANGLE.getKey().equalsIgnoreCase(shape)) && notInMeter <= 0)
							|| (SharpTypeEnum.LINE.getKey().equalsIgnoreCase(shape) && notInMeter <= LINEOFFSET)) {
						succ = true;
	...
	@RequestMapping(value = "loadCompanySectionTreeByPermission" + ManagementConstant.PERMISSION_SUFFIX_READ)
	public RestResultDto loadCompanySectionTreeByPermission(HttpServletRequest request) {
		String msg = null;
		Integer result = ManagementConstant.REST_RESULT_SUCC;
		Object data = null;
		try {
			...
			if (StringUtils.isNotEmpty(typeCode)) {
				...
				List<SearchFilter> typeFilters = Lists.newArrayList();
				typeFilters.add(new SearchFilter("code", Operator.LIKE, typeCode));
				typeFilters.add(new SearchFilter("tenantId", Operator.EQ, tenantId));
				List<WorkElementType> types = workElementTypeService.findListByFilter(typeFilters, null);
				if (CollectionUtils.isNotEmpty(types)) {
			...
			tree.reloadOrganizationElementTree(tenantId, userId, searchFilter);
	...
	public RestResultDto<?> getWorkElementsByCodes(String codes, String tenantId) {
		try {
			List<String> list = null;
			if (StringUtils.isNotEmpty(codes)) {
				list = Arrays.asList(codes.split(","));
			}
			return RestResultDto.newSuccess(this.workElementService.getWorkElementsByCodes(tenantId, list), "查询成功");
		} catch (Exception e) {
			String error_msg = "查询失败";
