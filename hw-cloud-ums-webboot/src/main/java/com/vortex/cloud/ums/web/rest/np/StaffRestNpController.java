...
public class StaffRestNpController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "delete" + ManagementConstant.PERMISSION_SUFFIX_READ, method = { RequestMethod.GET, RequestMethod.POST })
	@ResponseBody
	...
	public RestResultDto addStaff(HttpServletRequest request) throws Exception {
		...
		try {
			...
			if (StringUtils.isEmpty(dto.getTenantId())) {
				throw new VortexException("所属租户id不能为空");
			}
	...
	public RestResultDto getWillManStaffUser(HttpServletRequest request) throws Exception {
		...
		try {
			...
			String name = (String) paramMap.get("name");
			String willCheckDivisionIds = (String) paramMap.get("willCheckDivisionIds");
			String ignoreUserIds = (String) paramMap.get("ignoreUserIds");
			...
			if (null == num || 0 == num) {
				...
				// 页码请求从1开始，数据库从0开始
				pageNumber = pageNumber - 1;
				pageable = new PageRequest(pageNumber, pageSize, Direction.DESC, "createTime");
			}
			data = cloudStaffService.getWillManStaffUser(tenantId, name, willCheckDivisionIds, ignoreUserIds, num, pageable);
			msg = "查询成功！";
		} catch (Exception e) {
			e.printStackTrace();
			result = ManagementConstant.REST_RESULT_FAIL;
	...
	@RequestMapping(value = "addDtl" + ManagementConstant.PERMISSION_SUFFIX_SA, method = RequestMethod.POST)
	public RestResultDto<Boolean> addDtl(HttpServletRequest request, CloudStaffDto dto) {
		try {
	...
	@RequestMapping(value = "updateDtl" + ManagementConstant.PERMISSION_SUFFIX_SA, method = RequestMethod.POST)
	public RestResultDto<Boolean> updateDtl(HttpServletRequest request, CloudStaffDto staff) {
		try {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "syncStaffCount" + ManagementConstant.PERMISSION_SUFFIX_READ, method = { RequestMethod.GET, RequestMethod.POST })
	public RestResultDto<Long> syncStaffCount(HttpServletRequest request) throws Exception {
		JsonMapper mapper = new JsonMapper();
		try {
			Map<String, Object> paramMap = jm.fromJson(SpringmvcUtils.getParameter(ManagementConstant.REST_PMS), Map.class);
			if (MapUtils.isEmpty(paramMap)) {
			...
			return RestResultDto.newSuccess(cloudStaffService.syncStaffCount(tenantId, syncTime), "查询成功");
		} catch (Exception e) {
			logger.error("syncStaffCount", e);
			return RestResultDto.newFalid("查询失败", e.getMessage());
		}
	}
}
