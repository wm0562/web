...
public class AopService {
	...
	public Object aroundRequest(ProceedingJoinPoint joinpoint) throws Throwable {
		...
		// boolean hasPermission = this.hasPermission(userId, functionCode.value());
		// // 如果功能验证通过，则执行方法；否则直接返回错误
		// if (hasPermission) {
		// rst = joinpoint.proceed();
		// } else {
		// // 返回结果
	...
	private String getUserId(JoinPoint joinpoint) {
		...
		return StringUtils.isEmpty(SpringmvcUtils.getParameter("userId")) ? commonUtilsService.getUserId(request) : SpringmvcUtils.getParameter("userId");
	}
