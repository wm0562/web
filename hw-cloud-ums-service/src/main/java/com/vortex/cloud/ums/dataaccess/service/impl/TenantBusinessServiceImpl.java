...
public class TenantBusinessServiceImpl implements ITenantBusinessService {
	...
	public void copyResources(String tenantId, String cloudSystemId) {
		...
		String URL = cloudSystem.getWebsite() + ManagementConstant.URI_HEAD + "/" + systemCode.substring(systemCode.indexOf("_") + 1, systemCode.length()).toLowerCase() + "/" + ManagementConstant.COPY_RESOURCE_URI;
		Map<String, Object> prams = Maps.newHashMap();
		prams.put(ManagementConstant.TENANT_ID_KEY, tenantId);

		String result = "";
		try {
			result = ConnectHttpService.callHttp(URL, ConnectHttpService.METHOD_POST, prams); // 调用云系统资源拷贝的rest服务
		} catch (Exception e) {
			e.printStackTrace();
	...
	private List<CloudMenu> getMenuList(List<String> systemIds) {
		...
		searchFilters.add(new SearchFilter("systemId", Operator.IN, (String[]) systemIds.toArray(new String[systemIds.size()])));
		// 未隐藏的菜单
	...
	/**
	 * @Title: getTenantMap @Description: 获取功能信息 @return Map<String,Tenant> @throws
	 */
	private Map<String, CloudSystemFunctionDto> getFunctionMap(List<CloudMenu> menu_list) {
		...
		List<CloudSystemFunctionDto> function_list = (List<CloudSystemFunctionDto>) cloudFunctionDao.getFunctionsByIds(StringUtils.join(functionId_list, ","));
	...
	/**
	 * @Title: addTenantMenu @Description: //为每个租户管理员添加管理员菜单 @return void @throws
	 */
	private void addTenantMenu(String tenantId, Map<String, List<MenuTreeDto>> system_menu_map) {
		...
		String key = ManagementConstant.REDIS_PRE_SYS_MENU + ManagementConstant.REDIS_SEPARATOR + tenantId + ManagementConstant.REDIS_SEPARATOR + sys_code;
	...
	private void addDefaulSystemRoot(String userId, String systemId, String systemCode) {
		 ...
		 * newFun.setDescription("该功能在新增系统时自动生成"); newFun.setGroupId(funGroup.getId());
		 ...
		 * 
		 * for (CloudMenu oldMenu : oldMenus) { CloudMenu newMenu = new CloudMenu();
		 * BeanUtils.copyProperties(oldMenu, newMenu);
		 * newMenu.setId(UUIDGenerator.getUUID()); newMenu.setSystemId(systemId);
