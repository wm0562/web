...
public class CloudStaffServiceImpl extends SimplePagingAndSortingService<CloudStaff, String> implements ICloudStaffService {
	...
	public CloudStaff save(CloudStaffDto dto) {
		...
		return cloudStaffDao.save(entity);
	}
	...
	public Page<CloudStaffDto> findPageWithPermissionBySearchDto(Pageable pageable, CloudStaffSearchDto searchDto) {
		...
		if (searchDto.getCkRange().equals("false") && StringUtils.isNotEmpty(searchDto.getOrgId())) {
			companyIds = Lists.newArrayList(searchDto.getOrgId());
		} else if (searchDto.getCkRange().equals("false") && StringUtils.isNotEmpty(searchDto.getDepartmentId())) {
			companyIds = Lists.newArrayList(searchDto.getDepartmentId());
		} else {
			companyIds = cloudOrganizationService.getCompanyIdsWithPermission(userId, searchDto.getTenantId());
		}
	...
	@Override
	public List<CloudStaffDto> findListWithPermissionBySearchDto(Sort defSort, CloudStaffSearchDto searchDto) {
		// 如果没有传orgId或者部门id，那么就直接返回null
		if (StringUtils.isEmpty(searchDto.getDepartmentId()) && StringUtils.isEmpty(searchDto.getOrgId())) {
			return null;
		}
	...
	@Override
	public Object getWillManStaffUser(String tenantId, String name, String willCheckDivisionIds, String ignoreUserIds, Integer num, Pageable pageable) {
		Map<String, Object> returnValue = Maps.newHashMap();
		returnValue.put("total", 0);
		returnValue.put("rows", Lists.newArrayList());
		if (StringUtil.isNullOrEmpty(tenantId)) {
			return returnValue;
		}
		List<CloudStaffDto> staffList = Lists.newArrayList();
		if (null == num || 0 == num) {// 不抽取时分页
			Page<CloudStaffDto> page = cloudStaffDao.getWillManStaffUserPage(tenantId, name, willCheckDivisionIds, ignoreUserIds, pageable);
			staffList = page.getContent();
			returnValue.put("total", page.getTotalElements());
		} else {// 随机抽取
			List<CloudStaffDto> list = cloudStaffDao.getWillManStaffUserList(tenantId, name, willCheckDivisionIds, ignoreUserIds);
			...
			returnValue.put("total", staffList.size());
		}
		...
		List<Object> data_list = Lists.newArrayList();
		...
		for (CloudStaffDto staff : staffList) {
			...
			data_list.add(mapValue);
		}
		returnValue.put("rows", data_list);
		return returnValue;
	}
	...
	public Long syncStaffCount(String tenantId, Long syncTime) {
		return this.cloudStaffDao.syncStaffCount(tenantId, syncTime);
	}
}
