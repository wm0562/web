...
public class TenantDivisionTree extends CommonTree {
	 ...
	 *            用于在租户树下，过滤出子树
	 */
	public void reloadTenantDivisionTreeByLevel(String tenantId, Integer level) throws Exception {
		List<Object> nodes = new ArrayList<>();
		// 添加根
		CommonTreeNode root = generateRoot(tenantId, null);
		nodes.add(root);

		List<TenantDivision> list = this.findTenantDivisionList(tenantId, level);
		if (CollectionUtils.isNotEmpty(list)) {
			for (TenantDivision entity : list) {
				if (entity.getId().equals(root.getNodeId())) {
					continue;
				}
				nodes.add(entity);
			}
	...
	private List<TenantDivision> findTenantDivisionList(String tenantId, Integer level) {
		// 取租户下的所有节点
		ITenantDivisionService service = this.getTenantDivisionService();
		return service.getAllByLevel(tenantId, level);
	}
