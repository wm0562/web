...
public class CloudMenuServiceImpl extends SimplePagingAndSortingService<CloudMenu, String> implements ICloudMenuService {
	...
	public CloudMenu saveBusinessSystem(CloudMenuDto dto) {
		...
		// 设置nodecode
		dto.setNodeCode(this.cloudMenuDao.getNextNodeCode(dto.getParentId(), dto.getSystemId()));
	...
	public void updateForBusinessSystem(CloudMenuDto dto) {
		...
		String npid = dto.getParentId(); // 新的父节点
		String opid = old.getParentId(); // 旧的父节点
		String nodeCode = old.getNodeCode();
		if (!npid.equals(opid)) { // 如果父节点发送变化，则需改变树形字符串
			// 设置新父节点下的nodecode
			nodeCode = this.cloudMenuDao.getNextNodeCode(npid, old.getSystemId());
		...
		if (!npid.equals(opid)) {
			// 递归更新当前节点的所有下层节点的nodeCode
			this.updateChildrenNodeCode(dto.getId(), nodeCode, old.getSystemId());
		}
	}
	 ...
	 * @param nodeCode
	 */
	private void updateChildrenNodeCode(String pid, String nodeCode, String systemId) {
		List<CloudMenu> list = this.cloudMenuDao.getMenusByParentId(systemId, pid);
		if (CollectionUtils.isEmpty(list)) {
			return;
		}
		...
		for (int i = 0; i < list.size(); i++) {
			String ncode = null;
			if (i < 9) {
				ncode = nodeCode + "0" + (i + 1);
			} else {
				ncode = nodeCode + (i + 1);
			}
			list.get(i).setNodeCode(ncode);
			this.cloudMenuDao.update(list.get(i));

			// 递归
			updateChildrenNodeCode(list.get(i).getId(), ncode, systemId);
		}
	}
