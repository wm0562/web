...
public class TenantParamSettingServiceImpl extends SimplePagingAndSortingService<TenantPramSetting, String> implements ITenantParamSettingService {
	...
	public Map<String, List<Map<String, Object>>> loadMultiParamList(String tenantId, List<String> typeCodes) {
		...
		for (int i = 0; i < typeCodes.size(); i++) {
			typeCodeList.add(PropertyUtils.getPropertyValue(typeCodes.get(i)));
		}
		List<TenantPramSettingDto> tenantPramSettingDtos = tenantPramSettingDao.findByParamTypeCodes(tenantId, typeCodeList);
