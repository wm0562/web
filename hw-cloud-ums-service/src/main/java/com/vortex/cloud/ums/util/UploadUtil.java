...
public class UploadUtil {
	...
	private static ICentralCacheRedisService centralCacheRedisService = SpringContextHolder.getBean(ICentralCacheRedisService.CLASSNAME);
	private static ICommonUtilsService commonUtilsService = SpringContextHolder.getBean("commonUtilsService");
						...
						for (String xlsFileName : xlsList) {
							...
							for (ExcelSheetDTO sheetDTO : sheetList) {
								...
								for (ExcelRowDTO rowDTO : rowList) {
									...
									Map<String, Object> importFlagMap = uploadService.importData(entityTemp, tenantId, marks, rowIndex);
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static void uploadImportDataWithParam(HttpServletRequest request, HttpServletResponse response, Class uploadModelClass, IUploadService uploadService, Map<String, Object> personalParams) throws Exception {
		RestResultDto<String> oi = new RestResultDto();
		oi.setResult(RestResultDto.RESULT_SUCC);
		response.setContentType("text/html;charset=UTF-8");
		String[] params = new String[] { "alais" };
						...
						for (String xlsFileName : xlsList) {
							...
							for (ExcelSheetDTO sheetDTO : sheetList) {
								...
								for (ExcelRowDTO rowDTO : rowList) {
									...
									UploadTempModel entityTemp = ((UploadTempModel) uploadModelClass.newInstance()).storeCell(rowDTO);
									entityTemp.setFileName(fileName);
									// Map<String, Object> importFlagMap = uploadService.importData(entityTemp,
									// tenantId, marks, rowIndex);
									Map<String, Object> importFlagMap = uploadService.importDataWithParam(entityTemp, tenantId, marks, rowIndex, personalParams);
									if (importFlagMap.get("succFlag") != null && (Boolean) importFlagMap.get("succFlag")) {
										succCount++;
									} else {
										failCount++;
