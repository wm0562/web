...
public class CloudDepartmentServiceImpl extends SimplePagingAndSortingService<CloudDepartment, String> implements ICloudDepartmentService {
	...
	public CloudDepartment save(CloudDepartmentDto dto) {
		...
		return cloudDepartmentDao.save(department);
	}
	...
	private void setDeptTypeText(CloudDepartmentDto dto) {
		...
		TenantPramSetting tenantPramSetting = tenantParamSettingService.findOneByParamCode(dto.getTenantId(), PropertyUtils.getPropertyValue("DEPARTMENT_TYPE"), dto.getDepType());
	...
	public void update(CloudDepartmentDto newDto) {
		...
		// 组织名称没有改，不更新下面的用户orgName
		if (!oldDept.getDepName().equals(newDto.getDepName())) {
			jdbcTemplate.execute("UPDATE cloud_staff SET orgName = '" + newDto.getDepName() + "' WHERE departmentId = '" + oldDept.getId() + "' AND (orgId is null or orgId = '')");
		}
